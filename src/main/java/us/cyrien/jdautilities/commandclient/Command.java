...
public abstract class Command {
    ...
    public final void run(CommandEvent event) {
        ...
        if (event.isFromType(ChannelType.TEXT) && !isAllowed(event.getTextChannel())) {
            terminate(event, "`" + Locale.getCommandMessage("invalid-tc").finish() + "`");
            return;
        }
    ...
    /**
     * Checks whether a command is allowed in a {@link net.dv8tion.jda.core.entities.TextChannel TextChannelCmd}
     ...
     * in the first place.
     *
     * @param channel The TextChannelCmd to test.
     ...
     * @apiNote These are effective across a single instance of JDA, and not multiple ones.
